...
//  SwiftHelper (iOS)
//
//  Created by sauron on 2022/6/22.
...
//-----------------------------------------------------------------------------------------------------------------------------------------------
public extension SHProgressHUD {
...
public extension SHProgressHUD {
    ...
    //-------------------------------------------------------------------------------------------------------------------------------------------
    class func show( _ status: String? = nil, interaction: Bool = true, _ view: UIView? = nil) {
        
        DispatchQueue.main.async {
            shared.setup(view: view, status: status, hide: false, interaction: interaction)
        }
    }
    ...
    //-------------------------------------------------------------------------------------------------------------------------------------------
    @available(iOS 13.0, *)
    class func show(_ status: String? = nil, icon: AlertIcon, interaction: Bool = true, _ view: UIView? = nil) {
        
        let image = icon.image?.withTintColor(shared.colorAnimation, renderingMode: .alwaysOriginal)
        
        DispatchQueue.main.async {
            shared.setup(view: view, status: status, staticImage: image, hide: true, interaction: interaction)
        }
    }
    
    //-------------------------------------------------------------------------------------------------------------------------------------------
    class func show(_ status: String? = nil, icon animatedIcon: AnimatedIcon, interaction: Bool = true, _ view: UIView? = nil) {
        
        DispatchQueue.main.async {
            shared.setup(view: view, status: status, animatedIcon: animatedIcon, hide: true, interaction: interaction)
        }
    }
    
    // MARK: -
    //-------------------------------------------------------------------------------------------------------------------------------------------
    class func showSuccess(_ status: String? = nil, image: UIImage? = nil, interaction: Bool = true, _ view: UIView? = nil) {
        
        DispatchQueue.main.async {
            shared.setup(view: view, status: status, staticImage: image ?? shared.imageSuccess, hide: true, interaction: interaction)
        }
    }
    
    //-------------------------------------------------------------------------------------------------------------------------------------------
    class func showError(_ status: String? = nil, image: UIImage? = nil, interaction: Bool = true, _ view: UIView? = nil) {
        
        DispatchQueue.main.async {
            shared.setup(view: view, status: status, staticImage: image ?? shared.imageError, hide: true, interaction: interaction)
        }
    }
    
    // MARK: -
    //-------------------------------------------------------------------------------------------------------------------------------------------
    class func showSucceed(_ status: String? = nil, interaction: Bool = true, _ view: UIView? = nil) {
        
        DispatchQueue.main.async {
            shared.setup(view: view, status: status, animatedIcon: .succeed, hide: true, interaction: interaction)
        }
    }
    
    //-------------------------------------------------------------------------------------------------------------------------------------------
    class func showFailed(_ status: String? = nil, interaction: Bool = true, _ view: UIView? = nil) {
        
        DispatchQueue.main.async {
            shared.setup(view:view, status: status, animatedIcon: .failed, hide: true, interaction: interaction)
        }
    }
    
    //-------------------------------------------------------------------------------------------------------------------------------------------
    class func showAdded(_ status: String? = nil, interaction: Bool = true, _ view: UIView? = nil) {
        
        DispatchQueue.main.async {
            shared.setup(view: view, status: status, animatedIcon: .added, hide: true, interaction: interaction)
        }
    }
    class func showInfo(_ status: String? = nil, interaction: Bool = true, _ view: UIView? = nil) {
        
        DispatchQueue.main.async {
            shared.setup(view: view, status: status, animatedIcon: .info, hide: true, interaction: interaction)
        }
    }
    // MARK: -
    //--------------------------------showProgress-----------------------------------------------------------------------------------------------------------
    class func showProgress(_ progress: CGFloat, interaction: Bool = false, _ view: UIView? = nil) {
        
        DispatchQueue.main.async {
            shared.setup(view: view, progress: progress, hide: false, interaction: interaction)
        }
    }
    
    //-------------------------------------------------------------------------------------------------------------------------------------------
    class func showProgress(_ status: String?, _ progress: CGFloat, interaction: Bool = false, _ view: UIView? = nil) {
        
        DispatchQueue.main.async {
            shared.setup(view: view, status: status, progress: progress, hide: false, interaction: interaction)
        }
    }
}

//-----------------------------------------------------------------------------------------------------------------------------------------------
public class SHProgressHUD: UIView {
    
    private var viewBackground: UIView?
    private var toolbarHUD: UIToolbar?
    private var labelStatus: UILabel?
    
    private var viewProgress: ProgressUIView?
    ...
    private var imageSuccess: UIImage = {
        if #available(iOS 13.0, *) {
            ...
            return UIImage(named: "成功自定义图片") ?? UIImage()
        }
    }()
    private var imageError: UIImage    = {
        if #available(iOS 13.0, *) {
            ...
            return UIImage(named: "错误自定义图片") ?? UIImage()
        }
    }()
    ...
    //-------------------------------------------------------------------------------------------------------------------------------------------
    static let shared: SHProgressHUD = {
        let instance = SHProgressHUD()
        return instance
    } ()
    ...
    // MARK: -
    //-------------------------------------------------------------------------------------------------------------------------------------------
    private func setup(view: UIView? = nil, status: String? = nil, progress: CGFloat? = nil, animatedIcon: AnimatedIcon? = nil, staticImage: UIImage? = nil, hide: Bool, interaction: Bool) {
        
        setupNotifications()
        setupBackground(interaction,view)
        setupToolbar()
        setupLabel(status)
    ...
    //-------------------------------------------------------------------------------------------------------------------------------------------
    private func setupBackground(_ interaction: Bool, _ view: UIView? = nil) {
        
        if (viewBackground == nil) {
            if let view = view {
                viewBackground = UIView(frame: view.frame)
    ...
    private func setupProgress(_ progress: CGFloat?) {
        ...
        if (viewProgress == nil) {
            viewProgress = ProgressUIView(colorProgress)
    ...
    private func setupAnimatedIcon(_ animatedIcon: AnimatedIcon?) {
        ...
        if (animatedIcon == .info)        { animatedIconInfo(viewAnimatedIcon!)        }
    }
...
//        let delay = (self.alpha == 0) ? 0.25 : 0.0
        ...
        layer.strokeColor = UIColor(red: 48/255.0, green: 191/255.0, blue: 71/255.0, alpha: 1).cgColor
        layer.lineWidth = 9
        layer.lineCap = .round
    ...
    private func animatedIconFailed(_ view: UIView) {
        ...
        let length = view.frame.width
        //        let delay = (self.alpha == 0) ? 0.5 : 0.0
    ...
    //MARK:----------------------------------------info---------------------------------------------------------------------------------------------------
    private func animatedIconInfo(_ view: UIView) {
        ...
        // draw circle
        //        checkmarkShapePath.move(to: CGPoint(x: length*0.5, y: length*0.5+10))
        checkmarkShapePath.addArc(withCenter: CGPoint(x: length*0.5, y: length*0.5+10), radius: length*0.5, startAngle: 0, endAngle: CGFloat(Double.pi*2), clockwise: true)
        checkmarkShapePath.close()
        
        checkmarkShapePath.move(to: CGPoint(x: length*0.5, y: 25))
        checkmarkShapePath.addLine(to: CGPoint(x: length*0.5, y: 45))
        //        checkmarkShapePath.move(to: CGPoint(x: length*0.5, y: length*0.1))
        //        checkmarkShapePath.close()
        
        UIColor.white.setStroke()
        checkmarkShapePath.stroke()
        
        let checkmarkShapePathS = UIBezierPath()
        //        checkmarkShapePathS.move(to: CGPoint(x: length+0.5, y: length*0.5+10))
        checkmarkShapePathS.addArc(withCenter: CGPoint(x: length*0.5, y: length-10), radius: 2, startAngle: 0, endAngle: CGFloat(Double.pi*2), clockwise: true)
        checkmarkShapePathS.close()
        
        UIColor.white.setFill()
        checkmarkShapePathS.fill()
        let paths = [checkmarkShapePath,checkmarkShapePathS]
        ...
        for i in 0..<2 {
            ...
            layer.path = paths[i].cgPath
            layer.fillColor = UIColor.clear.cgColor
            layer.strokeColor = UIColor(red: 255/255.0, green: 190/255.0, blue: 0, alpha: 1).cgColor
            layer.lineWidth = 9
            layer.lineCap = .round
            layer.lineJoin = .round
            layer.strokeEnd = 0
...
// MARK: - ProgressUIView
//-----------------------------------------------------------------------------------------------------------------------------------------------
private class ProgressUIView: UIView {
    ...
    static private func systemBackground() -> UIColor {
        if #available(iOS 13.0, *) {
            return .systemBackground
        } else {
            return UIColor(red: 30/255.0, green: 144/255.0, blue: 255/255.0, alpha: 1)
        }
    }
    //-------------------------------------------------------------------------------------------------------------------------------------------
